...
이 8편의 성과 수치를 그대로 신뢰하기 어려운 이유:

| 논문들이 비교한 대상 | 실제 의미 |
|---------------------|-----------|
| SPT, EDD 등 1950년대 고전 룰 | 누가 만들어도 이김 |
| "GPT-4에게 그냥 물어보기" | 아무 최적화 없음, 당연히 이김 |
| 자기 방법에서 모듈만 뺀 ablation | "부품이 도움된다" 수준의 주장 |

8편 모두 스타일 C(자동화) 또는 D(파이프라인 개선)에 해당하며,
Gurobi, OR-Tools 등 실제 강한 베이스라인과 직접 비교한 논문이 없다.
→ `LLM_SCHEDULING_BREAKTHROUGH.md` 참조
...
**왜 LLM이 직접 스케줄링에 약한가 (3가지 근본 이유):**
1. **산술 계산 오류 누적**: 스케줄링은 수천 번의 정확한 연산이 필요한데, LLM은 토큰 예측으로 처리하므로 오류가 쌓인다
2. **제약 조건 추적 한계**: 작업 100개 × 기계 20대면 추적해야 할 제약이 수백 개. context window 안에서 완벽히 유지하기 어렵다
3. **조합 탐색 불가**: 최적해를 찾으려면 수백만 경우를 체계적으로 탐색해야 하는데, LLM은 "그럴듯한 답 하나"를 생성하는 방식이라 이 탐색을 할 수 없다
...
**단순 생성 vs Reflection의 차이:**
```
단순 Generation 프롬프트:
  "좋은 디스패칭 룰 만들어줘"
  → LLM의 사전 학습 지식에서 "그럴듯한" 답을 뽑아냄
  → 이 문제의 특성을 전혀 반영 못 함

Reflection 프롬프트:
  "A는 490, B는 520이 나왔어. 왜?"
  → LLM이 이 문제의 구체적 데이터를 분석
  → 이 인스턴스에 맞는 인사이트 추출
  → 다음 생성이 이 문제에 맞게 최적화됨
```

SeEvo는 Reflection을 3단계로 세분화한다:
- **개체 간 비교 반성**: "왜 A가 B보다 좋았나" → 이유 있는 crossover
- **자기 진화 반성**: 같은 개체의 버전 히스토리 추적 → 실패 원인 분석 (SeEvo 고유)
- **집단 반성**: 전체 세대의 성공/실패 패턴 종합 → 장기 기억 역할
